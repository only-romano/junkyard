...
const store = new Vuex.Store({
  state: {
    ...
    restartMessage: "Restart",  // button text

    expeditions: 0,  // user points counter
    markClose: 0,  // messages flag
  ...
  mutations: {
    clickListener: state => {
      // Initial event listener for body clicks (dispatches expedition)
    ...
    createMark: (state, evt) => {
      // Find x and y of user's point, distance to treasure point
      // dispatch - draw user point
    ...
    drawMark: (state, point) => {
      ...
      let markClose = (element, color, obj, value) => {
        // Sets border color of user's point, appends it to body
        // dispatches - set messages depends of close value
      ...
      else if (toMark <= 25) store.dispatch('winGame');  // game over, user wins
    },

    expedition: (state, evt) => {
      // increments user points counter, updates expedition message
      // dispatches - user's point drawing, game over (depends of counter)
    ...
    gameOver: state => {
      // end game flag, messages update, disclaimer showned
      state.game = false;
    ...
    restart: state => {
      // restore default values and game start over again
    ...
    setMessages: (state) => {
      // messages setting depends of messages flag
    ...
    winGame: state => {
      // draws red cross, update messages, game ended flag
...
const treasureApp = new Vue({
  ...
  mounted: () => {
    // after mounted dispatches body click listener and goal location setter
